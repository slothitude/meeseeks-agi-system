...
async function test() {
  ...
  const result = await helper.sendMessage(CHAT_ID, 'Button Test - Click a color!', JSON.stringify(keyboard));
  console.log('Sent! Message ID:', result.message_id);
  
  // Start polling for callbacks
  console.log('Waiting for button click (30 seconds)...');
  ...
  if (handled) {
    ...
    console.log('Timeout - no button clicked');
  }
}
